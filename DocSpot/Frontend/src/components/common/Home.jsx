...
const Home = () => {
  ...
  const fetchDoctors = async () => {
    try {
      const response = await axios.get('http://localhost:5000/api/user/getalldoctors');
      if (response.data.success) {
        const approvedDocs = response.data.data.filter(doc => doc.status === 'approved');
  ...
  return (
    <>
      ...
      <Container>
        ...
        <h2 className="text-center mt-5 mb-3">Our Doctors</h2>
        {loading ? (
                ...
                <Card>
                  <Card.Body>
                    ...
                    <Card.Text>
                      ...
                      <strong>Fees:</strong> ₹{doctor.fees}<br />
                      <strong>Timing:</strong> {doctor.timings[0]} - {doctor.timings[1]}
                    </Card.Text>
